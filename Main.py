...
# --- КОНФИГУРАЦИЯ ---
# Bot Token (BotFather-ден алған)
API_TOKEN = '8285487433:AAHYPgB_wsoRtoDpM1GwdyNPoAZG6Fj05Ug' 
# CryptoBot API (Төлемдер үшін)
CRYPTO_PAY_TOKEN = '535427:AAUe3CLI9OeKRaXrW61dQgHvB7kLLtNPfXb' 
# Өзіңнің Telegram ID-іңді осы жерге жаз (Чектер саған келуі үшін)
ADMIN_ID = 8145949506
...
# --- БАЗАНЫ БАПТАУ ---
def init_db():
    conn = sqlite3.connect('delayed_call.db')
    cur = conn.cursor()
    # state: 0 - төлем күту, 1 - номер күту, 2 - хат күту
...
STRINGS = {
    'kz': {
        'start': "👋 @DelayedCall сервисіне қош келдіңіз. Болашаққа аманат қалдыру үшін тарифті таңдаңыз:",
        'pay_msg': f"💳 **ТӨЛЕМ:** 1000 ₸\nКарта: `{CARD_NUMBER}`\nИесі: {CARD_HOLDER}\n\nАударып болған соң, чектің **скриншотын** жіберіңіз.",
        'ask_phone': "📱 **1-ші қадам:**\nЕнді хабарласатын телефон номерін жазыңыз (мысалы: +77071234567):",
    ...
    'ru': {
        'start': "👋 Добро пожаловать в @DelayedCall. Выберите тариф:",
...
async def set_lang(c: types.CallbackQuery):
    ...
    await bot.send_message(c.from_user.id, STRINGS[lang]['start'], reply_markup=kb)
...
# --- ДЕРЕКТЕРДІ ҚАДАМ-ҚАДАММЕН ҚАБЫЛДАУ ---
